...
服务器镜像列表只能解决 GitHub 不可达问题。如果第三方直播源也不可达，才考虑为少量频道增加代理。完整代理需要处理 HLS 清单重写、媒体分片、Token、请求头、带宽和延迟，不进入第一版范围。
...
| 环境 | 验证内容 |
|---|---|
| TypeScript 单元测试 | M3U 解析、频道合并、字段转换、源排序 |
| 服务器集成测试 | 下载失败、格式错误、原子发布、上一版本保留 |
...
- 服务器使用 TypeScript，同步并规范化 `iptv-org` CN 数据。
- App 只从自有服务器获取频道配置，并直接播放第三方直播源。
- 第一阶段先完成“同步、缓存、列表、播放、遥控、自动切源”闭环。
